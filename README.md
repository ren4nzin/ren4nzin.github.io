...
<title>Jardim da Contagem — Jogo da Memória</title>
<style>
  ...
  * { box-sizing: border-box; }
  ...
  input:focus, button:focus { outline: 3px solid #5AAFCB; outline-offset: 2px; }
...
// ---------- Theme: "Jardim da Contagem" (Counting Garden) ----------
const INK = "#2F2A44";
const CREAM = "#FFF9EE";
const CORAL = "#FF7A5C";
const LEAF = "#4CAF7D";
...
const CRITTERS = [
  { n: 1, icon: "🐌", name: "caracol" },
  { n: 2, icon: "🐞", name: "joaninha" },
  { n: 3, icon: "🕷️", name: "aranha" },
  { n: 4, icon: "🦋", name: "borboleta" },
  { n: 5, icon: "🐝", name: "abelha" },
  { n: 6, icon: "🐸", name: "sapo" },
];
...
function buildDeck() {
  const cards = [];
  CRITTERS.forEach((c) => {
    cards.push({ id: `num-${c.n}`, pairId: c.n, kind: "number", n: c.n });
    cards.push({ id: `img-${c.n}`, pairId: c.n, kind: "image", n: c.n, icon: c.icon });
  });
  return shuffle(cards).map((c, i) => ({ ...c, key: i, matched: false }));
}
...
function App() {
  ...
  const [mathAsked, setMathAsked] = useState(false);
  ...
  const totalPairs = CRITTERS.length;

  useEffect(() => {
    if (screen === "game" && !mathAsked && pairsMatched >= 3 && idadeNum >= 7) {
      setMathQuestion(makeMathQuestion());
      setShowMath(true);
      setMathAsked(true);
      setLock(true);
    }
  }, [pairsMatched, mathAsked, idadeNum, screen]);

  useEffect(() => {
    if (screen === "game" && pairsMatched === totalPairs && deck.length > 0) {
      clearInterval(timerRef.current);
      const t = setTimeout(() => setScreen("results"), 900);
      return () => clearTimeout(t);
    }
  }, [pairsMatched, totalPairs, deck, screen]);
  ...
  const answerMath = (opt) => {
    setMathResult(opt === mathQuestion.answer);
    setTimeout(() => {
      setShowMath(false);
      setLock(false);
    }, 1400);
  ...
  return (
    <div style={{
      ...
      {screen === "form" && (
        <div style={{
          background: CARD_BACK, borderRadius: 28, padding: "40px 36px",
          maxWidth: 440, width: "100%", boxShadow: "0 20px 50px rgba(47,42,68,0.15)",
          ...
          <button onClick={startGame} className="wobble" style={{
            marginTop: 24, width: "100%", padding: "16px", borderRadius: 16, border: "none",
            background: CORAL, color: "#fff", fontFamily: "'Baloo 2', sans-serif", fontSize: 20,
            fontWeight: 700, cursor: "pointer", boxShadow: "0 8px 0 #D9573F", transition: "transform 0.15s ease",
          }}>
            Começar a jogar 🌻
          </button>
        </div>
      ...
      {screen === "game" && (
        <div style={{ width: "100%", maxWidth: 640 }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 14, flexWrap: "wrap", gap: 10 }}>
            <div>
              <div className="display-font" style={{ fontSize: 22, fontWeight: 700, color: INK }}>
                Oi, {nome.split(" ")[0]}! 🌿
              </div>
              <div style={{ fontSize: 13, color: "#8A83A0" }}>Encontre os pares de números e bichinhos</div>
            </div>
          ...
          <div style={{ display: "flex", gap: 6, marginBottom: 16, background: "#fff", borderRadius: 14, padding: "10px 14px", border: `2px solid #EFE7D6` }}>
            {CRITTERS.map((c, i) => (
              <div key={c.n} className={i < pairsMatched ? "bloom" : ""} style={{ flex: 1, textAlign: "center", fontSize: 22, opacity: i < pairsMatched ? 1 : 0.25 }}>
                {i < pairsMatched ? "🌸" : "⚪"}
              </div>
            ))}
          </div>
              ...
              return (
                <div key={card.key} className={`flip-card ${isFlipped ? "flipped" : ""}`}
                  ...
                  <div className="flip-inner">
                    <div className="flip-face" style={{ background: `linear-gradient(135deg, ${LEAF}, #3E9468)`, fontSize: 26 }}>🍃</div>
                    <div className="flip-face flip-back" style={{ background: isMatched ? "#EAF9EE" : "#fff", border: `2px solid ${isMatched ? LEAF : "#EEE6D6"}`, flexDirection: "column" }}>
                      {card.kind === "number" ? (
                        <span className="display-font" style={{ fontSize: 34, fontWeight: 800, color: CORAL }}>{card.n}</span>
                      ) : (
                        <div style={{ display: "grid", gridTemplateColumns: `repeat(${Math.min(card.n, 3)}, 1fr)`, gap: 2, padding: 6 }}>
                          {Array.from({ length: card.n }).map((_, i) => (
                            <span key={i} style={{ fontSize: card.n > 4 ? 13 : 16 }}>
                              {CRITTERS.find((c) => c.n === card.n).icon}
                            </span>
                          ))}
                        </div>
                      )}
      ...
      {screen === "results" && (
        <div style={{
          background: CARD_BACK, borderRadius: 28, padding: "40px 36px", maxWidth: 440, width: "100%",
          textAlign: "center", boxShadow: "0 20px 50px rgba(47,42,68,0.15)", border: `3px solid ${LEAF}`,
        }}>
          <div style={{ fontSize: 54, marginBottom: 8 }}>🏆🌼</div>
          <h1 className="display-font" style={{ fontSize: 28, color: LEAF, margin: "0 0 4px" }}>
            Muito bem, {nome.split(" ")[0]}!
          </h1>
          <p style={{ color: "#8A83A0", margin: "0 0 24px" }}>Você completou o Jardim da Contagem</p>

          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10, marginBottom: 18 }}>
            <Stat label="Idade" value={idade} />
            <Stat label="Data" value={data.split("-").reverse().join("/")} />
            <Stat label="Pares certos" value={`${totalPairs}/${totalPairs}`} />
            <Stat label="Jogadas" value={moves} />
            <Stat label="Tempo" value={formatTime(seconds)} />
            {mathAsked && <Stat label="Conta" value={mathResult ? "Acertou! 🎉" : "Quase lá!"} />}
          </div>

          <button onClick={playAgain} className="wobble" style={{
            width: "100%", padding: "16px", borderRadius: 16, border: "none", background: CORAL,
            color: "#fff", fontFamily: "'Baloo 2', sans-serif", fontSize: 18, fontWeight: 700,
            cursor: "pointer", boxShadow: "0 8px 0 #D9573F",
          }}>
            Jogar de novo 🔁
          </button>
        </div>
